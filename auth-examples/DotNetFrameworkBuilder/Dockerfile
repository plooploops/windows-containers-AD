# escape=`
FROM myplooploops/dotnet-framework:4.7.2-runtime-windowsservercore-insider-10.0.17666.1000

SHELL ["powershell", "-Command", "$ErrorActionPreference = 'Stop'; $ProgressPreference = 'SilentlyContinue';"]

#Install NuGet CLI
ENV NUGET_VERSION 4.4.1
RUN New-Item -Type Directory -Force $Env:ProgramFiles\NuGet; `
...
# Install VS Build Tools
    ...
    try { Remove-Item -Force -Recurse \"${Env:ProgramData}\Package Cache\" } catch { Write-Host $_.Exception.Message } 

# Install web targets.
# Note the extra path for v11.0 build.
RUN Invoke-WebRequest -UseBasicParsing https://dotnetbinaries.blob.core.windows.net/dockerassets/MSBuild.Microsoft.VisualStudio.Web.targets.2018.05.zip -OutFile MSBuild.Microsoft.VisualStudio.Web.targets.zip;`
    Expand-Archive -Force MSBuild.Microsoft.VisualStudio.Web.targets.zip -DestinationPath \"${Env:ProgramFiles(x86)}\Microsoft Visual Studio\2017\BuildTools\MSBuild\Microsoft\VisualStudio\v15.0\"; `
    Expand-Archive -Force MSBuild.Microsoft.VisualStudio.Web.targets.zip -DestinationPath \"${Env:ProgramFiles(x86)}\MSBuild\Microsoft\VisualStudio\v11.0\"; `
...
RUN try { \Windows\Microsoft.NET\Framework64\v4.0.30319\ngen update; `
    ...
    \Windows\Microsoft.NET\Framework\v4.0.30319\ngen update } catch { Write-Host $_.Exception.Message }

# Set PATH in one layer to keep image size down.
RUN setx /M PATH $(${Env:PATH} `
    + \";C:\Windows\Microsoft.NET\Framework64\v4.0.30319\" `
    + \";${Env:ProgramFiles(x86)}\MSBuild\Microsoft\VisualStudio\v11.0\" `
