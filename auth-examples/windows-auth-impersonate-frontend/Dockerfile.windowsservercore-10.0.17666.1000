...
# Build files
WORKDIR C:\src
COPY windows-auth-impersonate-frontend\packages.config .
RUN nuget restore packages.config -PackagesDirectory ..\packages

COPY . C:\src
RUN msbuild windows-auth-impersonate-frontend\windows-auth-impersonate-frontend.csproj /p:OutputPath=C:\out /p:DeployOnBuild=true 
...
COPY --from=build-agent C:\out\_PublishedWebsites\windows-auth-impersonate-frontend .
