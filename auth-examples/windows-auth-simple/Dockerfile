...
# Build files
WORKDIR C:\src
COPY windows-auth-simple\packages.config .
RUN nuget restore packages.config -PackagesDirectory ..\packages

COPY . C:\src
RUN msbuild windows-auth-simple\windows-auth-simple.csproj /p:OutputPath=C:\out /p:DeployOnBuild=true 
...
COPY --from=build-agent C:\out\_PublishedWebsites\windows-auth-simple .
